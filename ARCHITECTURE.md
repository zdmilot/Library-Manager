...
graph TB
    ...
    subgraph External
        ...
        REGASM["32-bit RegAsm.exe<br/>(COM DLL Registration)<br/>C:\Windows\Microsoft.NET\Framework\"]
...
> **32-bit Requirement:** Hamilton VENUS 6 is a 32-bit (x86) application. All COM DLL
> registration **must** use the 32-bit `RegAsm.exe` from
> `C:\Windows\Microsoft.NET\Framework\` — **never** `Framework64\`. The 64-bit
> `RegAsm.exe` registers COM objects in the 64-bit registry hive, which is invisible
> to 32-bit VENUS and will cause "Class not registered" runtime errors.
...
flowchart TD
    ...
    subgraph "Installation (installPackage)"
        ...
        COM_CHECK -->|"Yes (GUI)"| COM_REG["UAC-elevated 32-bit RegAsm.exe<br/>C:\Windows\Microsoft.NET\Framework\<br/>via PowerShell Start-Process<br/>── Never Framework64 ──"]
...
flowchart TD
    ...
    subgraph "File Cleanup"
        ...
        COM_DEREG -->|"Yes (GUI)"| UNREG["Deregister COM DLLs<br/>via UAC 32-bit RegAsm /unregister<br/>C:\Windows\Microsoft.NET\Framework\"]
