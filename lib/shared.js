...
/** Standard file extensions for Library Manager package files. */
const PKG_FILE_EXT = '.hxlibpkg';
const ARC_FILE_EXT = '.hxlibarch';
...
const KNOWN_MANIFEST_KEYS = [
    ...
    'labware_files','bin_files','bin_com_register_dlls','dependencies',
...
/**
 * Valid package categories for marketplace grouping.
 * The first entry is the default when no category is specified.
 */
const PACKAGE_CATEGORIES = ['General Utility', 'Device Driver'];
...
const KNOWN_LIB_DB_KEYS = [
    ...
    'demo_method_files','help_files','com_register_dlls','bin_com_register_dlls','labware_files',
...
const RESTRICTED_FILE_EXTENSIONS = [
    ...
    '.cab', '.sfx',
    // Compressed archives (can smuggle executables)
...
/** Maximum allowed container payload size (500 MB) */
const MAX_CONTAINER_SIZE = 500 * 1024 * 1024;
...
function unpackContainer(containerBuffer, magic) {
    ...
    if (payloadLen > MAX_CONTAINER_SIZE) {
        throw new Error('Invalid package: payload size (' + Math.round(payloadLen / 1048576) + ' MB) exceeds maximum allowed (' + Math.round(MAX_CONTAINER_SIZE / 1048576) + ' MB).');
    }
...
function sanitizeManifestPath(relPath) {
    ...
    if (/[\x00-\x1f\x7f\x80-\x9f\u200e\u200f\u202a-\u202e\u2066-\u2069]/.test(relPath)) {
        return { valid: false, sanitized: null, error: 'Path contains control characters or Unicode overrides: ' + relPath };
    }
    ...
    for (var i = 0; i < segments.length; i++) {
        ...
        if (/^(CON|PRN|AUX|NUL|COM[0-9]|LPT[0-9])(\..*)?$/i.test(seg)) {
...
function safeZipExtractPath(baseDir, fname) {
    ...
    // Reject UNC paths before any normalization (\\server\share)
    if (/^[\\/]{2}/.test(fname)) return null;
...
function verifyPackageSignature(zip) {
    ...
    if (typeof sig.file_hashes !== 'object' || Array.isArray(sig.file_hashes)) {
        result.valid = false;
        result.errors.push('signature.json file_hashes is not a valid object.');
        return result;
    }

    // Validate HMAC format before comparison
    if (typeof sig.hmac !== 'string' || !/^[a-f0-9]{64}$/i.test(sig.hmac)) {
        result.valid = false;
...
function sanitizeTagDetailed(tag) {
    ...
    // Strip Unicode whitespace (zero-width spaces, non-breaking spaces, etc.)
    var stripped = tag.replace(/[\u200B\u200C\u200D\uFEFF\u00A0\u2000-\u200A\u2028\u2029\u202F\u205F\u3000]/g, ' ');
    var normalized = stripped.trim().toLowerCase();
    if (!normalized) {
        result.blocked = true;
        result.blockCode = 'empty';
        result.blockReason = buildTagBlockReason('empty');
...
function validateGitHubRepoUrl(url) {
    ...
    // Reject unknown extra path segments — too permissive to accept arbitrary paths
    return { valid: false, reason: 'URL has unrecognized path segments after /{owner}/{repo}.' };
}
...
module.exports = {
    // Constants
    FORMAT_VERSION:     FORMAT_VERSION,
    PKG_FILE_EXT:       PKG_FILE_EXT,
    ARC_FILE_EXT:       ARC_FILE_EXT,
