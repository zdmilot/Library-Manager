...
const KNOWN_LIB_DB_KEYS = [
    ...
    'lib_install_path','demo_install_path','labware_install_path','installed_date','installed_by',
...
function validateManifestPaths(manifest) {
    var errors = [];
    var pathFields = ['library_files', 'demo_method_files', 'help_files', 'labware_files'];
...
function isValidAuthorName(name) {
    ...
    if (!SAFE_NAME_REGEX.test(name))
        return { valid: false, reason: 'Author contains invalid characters. Only letters, numbers, spaces, dashes, and underscores are allowed.' };
    return { valid: true };
}
...
function isValidOrganizationName(name) {
    ...
    if (!SAFE_NAME_REGEX.test(name))
        return { valid: false, reason: 'Organization contains invalid characters. Only letters, numbers, spaces, dashes, and underscores are allowed.' };
    return { valid: true };
}
