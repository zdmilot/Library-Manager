...
function checkSpam(text, username) {
    ...
    // Repeated characters (same char 8+ times in a row)
    if (/(.)(\1){7,}/.test(text)) return 'Repeated characters';
    // Very short spam-like review with URL
    if (text.length < 20 && urlCount > 0) return 'Suspicious short message with URL';
    return null;
}
