...
function decompressEntry(buf, entry) {
    ...
    if (data.length !== uncompressedSize) {
        throw new Error('Entry ' + entry.id + ' size mismatch: expected ' +
            uncompressedSize + ', got ' + data.length + '. Data may be corrupted.');
    }
    return data;
}
...
function parseManifest(manifest) {
    // Validate buffer structure before processing
    if (!Buffer.isBuffer(manifest) || manifest.length < 8) {
        return {};
    }
    // Check that the manifest contains at least one entry ID key marker
    if (manifest.indexOf(KEY_ENTRY_ID) === -1) {
        return {};
    }
    var map = {};
    var pos = 0;
    while (pos < manifest.length) {
        ...
        if (idLen > 0 && idLen < 20 && afterId + 1 + idLen <= manifest.length) {
            ...
            if (/^[0-9a-f]+$/.test(entryId)) {
                // Search for abs path key within 300 bytes forward (max HxPars record span)
