...
function getVENUSVersion() {
    ...
    try {
        ...
        for (var rp of regPaths) {
            try {
                var subkeysRaw = execFileSync('reg', ['query', rp], { encoding: 'utf8', timeout: REG_QUERY_TIMEOUT });
                var subkeys = subkeysRaw.split('\n').map(function(l){return l.trim();}).filter(function(l){return l.length > 0;});
                for (var sk of subkeys) {
                    try {
                        var entryRaw = execFileSync('reg', ['query', sk, '/v', 'DisplayName'], { encoding: 'utf8', timeout: REG_QUERY_TIMEOUT });
                        if (!/Hamilton\s+VENUS\s+\d/i.test(entryRaw)) continue;
                        var allVals = execFileSync('reg', ['query', sk], { encoding: 'utf8', timeout: REG_QUERY_TIMEOUT });
...
function autoAddToGroup(db, savedLibId, authorName) {
    try {
        ...
        if (shared.isRestrictedAuthor(authorName)) {
            ...
            var oemTreeEntry = db.tree.findOne({ 'group-id': 'gOEM' });
            if (oemTreeEntry) {
                targetGroupId = 'gOEM';
                var ids = oemTreeEntry['method-ids'] || [];
                if (ids.indexOf(savedLibId) === -1) {
                    ids.push(savedLibId);
            ...
            for (var j = 0; j < navtree.length; j++) {
                ...
                if (gEntry && !gEntry['default']) {
                    ...
                    // Re-read the specific tree entry fresh to minimize stale data
                    var freshEntry = db.tree.findOne({ 'group-id': targetGroupId });
                    var mids = (freshEntry && freshEntry['method-ids']) || [];
                    if (mids.indexOf(savedLibId) === -1) {
...
/**
 * Extract ZIP entries to their destination directories.
 * @param {AdmZip} zip - The ZIP archive to extract from
 * @param {string} libDestDir - Destination for library files
 * @param {string} demoDestDir - Destination for demo method files
 * @returns {number} Number of files extracted
 */
function extractZipEntries(zip, libDestDir, demoDestDir) {
    var extractedCount = 0;
    var prefixMap = {
        'library/':      libDestDir,
        'demo_methods/': demoDestDir
    };
    zip.getEntries().forEach(function(entry) {
        ...
        for (var prefix in prefixMap) {
            if (entry.entryName.startsWith(prefix)) {
                var fname = entry.entryName.substring(prefix.length);
                if (fname) {
                    var safePath = shared.safeZipExtractPath(prefixMap[prefix], fname);
                    if (!safePath) return;
...
function installPackage(manifest, zip, libDestDir, demoDestDir, sourceName, db, skipGroup, sigResult, installToRoot) {
    ...
    // Validate restricted file extensions in manifest
    var extCheck = shared.validateManifestFileExtensions(manifest);
    if (!extCheck.valid) {
        throw new Error('Package contains restricted file types: ' + extCheck.errors.join('; '));
    }
    ...
    var extractedCount = extractZipEntries(zip, libDestDir, demoDestDir);
    ...
    return { extractedCount: extractedCount, libName: manifest.library_name, libId: saved._id, libInstallPath: libDestDir, sigResult: sigResult || null };
}
...
function resolveSigningCredentials(signKeyPath, signCertPath) {
    ...
    if (signKeyPath && !signCertPath) {
        ...
        // Derive cert path from key path using naming convention, with explicit existence check
        var cp = kp.replace(/\.key\.pem$/i, '.cert.json');
        if (cp === kp) {
            // Key file did not match expected *.key.pem pattern — cannot derive cert path
            throw new Error('--sign-key file must end with .key.pem to auto-detect .cert.json, or specify --sign-cert explicitly.');
        }
        if (!fs.existsSync(cp)) throw new Error('--sign-key specified but no matching .cert.json found at: ' + cp);
 ...
 */
function getLibrary(ctx, optsOrNameOrId) {
    var nameOrId = (typeof optsOrNameOrId === 'string') ? optsOrNameOrId
                 : (optsOrNameOrId && (optsOrNameOrId.nameOrId || optsOrNameOrId.name || optsOrNameOrId.id)) || null;
...
function importLibrary(ctx, opts) {
    ...
    var svcExtValidation = shared.validateManifestFileExtensions(manifest);
    if (!svcExtValidation.valid) {
        return { success: false, error: 'Package contains restricted file types that could pose a security risk: ' + svcExtValidation.errors.join('; ') };
    }

    // Advisory lock: serialize check-then-install to prevent race conditions (BUG-03)
    return advisoryLock.withLock(ctx.dbPath, 'db-write', function() {
        ...
        var svcCustomSubdir = (manifest.custom_install_subdir) ? manifest.custom_install_subdir : '';
        var libDestDir;
        if (svcCustomSubdir) {
            libDestDir = path.join(ctx.libBasePath, svcCustomSubdir);
        } else {
            libDestDir = ctx.libBasePath;
        }
        var demoDestDir = path.join(ctx.metBasePath, 'Library Demo Methods', libName);
        var svcInstallToRoot = !svcCustomSubdir;
        ...
        try {
            appendAuditTrailEntry(ctx.dbPath, buildAuditTrailEntry('library_imported', {
                ...
                signature_override: signatureStatus === 'failed' && !!opts.force
            }));
        } catch (_) {}
...
function importArchive(ctx, opts) {
    ...
    pkgEntries.forEach(function(pkgEntry) {
        ...
        try {
            ...
            advisoryLock.withLock(ctx.dbPath, 'db-write', function() {
                ...
                var archSvcCustomSubdir = (manifest.custom_install_subdir) ? manifest.custom_install_subdir : '';
                var libDestDir;
                if (archSvcCustomSubdir) {
                    libDestDir = path.join(ctx.libBasePath, archSvcCustomSubdir);
                } else {
                    libDestDir = ctx.libBasePath;
                }
                var demoDestDir = path.join(ctx.metBasePath, 'Library Demo Methods', libName);
                var archSvcInstallToRoot = !archSvcCustomSubdir;
...
function exportLibrary(ctx, opts) {
    ...
    var outDir = path.dirname(path.resolve(opts.output));
    try {
        fs.accessSync(outDir, fs.constants.W_OK);
    } catch (_) {
        return { success: false, error: 'Output directory is not writable: ' + outDir };
    }
...
function exportArchive(ctx, opts) {
    ...
    targetLibs.forEach(function(lib) {
        try {
            ...
            if (lib.install_to_library_root) manifest2.install_to_library_root = true;
            if (lib.custom_install_subdir)   manifest2.custom_install_subdir = lib.custom_install_subdir;
            Object.keys(lib).forEach(function(k) {
    ...
    return advisoryLock.withLock(ctx.dbPath, 'db-write', function() {
        ...
        if (!opts.keepFiles) {
            ...
            if (libPath && (libFiles.length > 0 || helpFiles.length > 0)) {
                libFiles.concat(helpFiles).forEach(function(f) {
                    try {
                        ...
                        // Use lstatSync to detect (and safely remove) symlinks without following them
                        try { fs.lstatSync(fp); } catch(_) { return; }
                        fs.unlinkSync(fp);
                    } catch(_){}
                });
                try {
                    // Use lstatSync to check path without following symlinks
                    try { var stat = fs.lstatSync(libPath); } catch(_) { stat = null; }
                    if (stat && stat.isDirectory() && fs.readdirSync(libPath).length === 0) {
            ...
            if (demoPath && demoFiles.length > 0) {
                demoFiles.forEach(function(f) {
                    try {
                        var fp2 = path.join(demoPath, f);
                        try { fs.lstatSync(fp2); } catch(_) { return; }
                        fs.unlinkSync(fp2);
                    } catch(_){}
                });
                try {
                    try { var stat2 = fs.lstatSync(demoPath); } catch(_) { stat2 = null; }
                    if (stat2 && stat2.isDirectory() && fs.readdirSync(demoPath).length === 0) {
...
function createPackage(ctx, opts) {
    ...
    if (developerOnlyFields.length > 0) {
        return { success: false, error: 'Developer-only fields cannot be used via the COM interface: ' + developerOnlyFields.join(', ') + '. These features are restricted to the GUI application and require OEM authorization.' };
    }
    ...
    if (spec.tags && Array.isArray(spec.tags)) {
        ...
        spec.tags = shared.sanitizeTags(tagCheck.filtered);
    }
    ...
    // Validate restricted file extensions
    var allCreateFiles = resolvedLibFiles.concat(resolvedDemoFiles);
    var restrictedCreateFiles = allCreateFiles.filter(function(f) { return shared.isRestrictedFileExtension(f); });
    if (restrictedCreateFiles.length > 0) {
        return { success: false, error: 'Restricted file types cannot be included in library packages: ' + restrictedCreateFiles.map(function(f) { return path.basename(f); }).join(', ') };
    }
    ...
    if (resolvedImagePath && fs.existsSync(resolvedImagePath)) {
        var imgStat = fs.statSync(resolvedImagePath);
        if (imgStat.size > MAX_IMAGE_SIZE) {
            return { success: false, error: 'Library image exceeds maximum size of ' + Math.round(MAX_IMAGE_SIZE / 1048576) + ' MB.' };
        }
        ...
        if (bmpFound) {
            var bmpStat = fs.statSync(bmpFound);
            if (bmpStat.size <= MAX_IMAGE_SIZE) {
    ...
    if (spec.install_to_library_root) manifest.install_to_library_root = true;
    if (spec.custom_install_subdir)   manifest.custom_install_subdir = spec.custom_install_subdir;
...
function rollbackLibrary(ctx, opts) {
    ...
    return advisoryLock.withLock(ctx.dbPath, 'db-write', function() {
        var rbSvcCustomSubdir = (manifest.custom_install_subdir) ? manifest.custom_install_subdir : '';
        var libDestDir;
        if (rbSvcCustomSubdir) {
            libDestDir = path.join(ctx.libBasePath, rbSvcCustomSubdir);
        } else {
            libDestDir = ctx.libBasePath;
        }
        var demoDestDir = path.join(ctx.metBasePath, 'Library Demo Methods', opts.name);
        var rbSvcInstallToRoot = !rbSvcCustomSubdir;
