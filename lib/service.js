...
function installPackage(manifest, zip, libDestDir, demoDestDir, sourceName, db, skipGroup, sigResult, installToRoot) {
    ...
    // When installToRoot is false, the libDestDir already includes the library name
    // or custom subdir, so we must strip that prefix from ZIP entry paths to prevent
    // double-nesting (e.g. Library/LibName/LibName/file.hsl).
    var libName = manifest.library_name || '';
    var customSubdir = manifest.custom_install_subdir || '';
    var importStripPrefix = '';
    if (!installToRoot && (libName || customSubdir)) {
        importStripPrefix = (customSubdir || libName).replace(/\\/g, '/').replace(/^\/+|\/+$/g, '');
    }
     ...
     * This handles backward compatibility with packages that included the
     * library name in ZIP paths.
     */
    function stripImportPrefix(fname) {
        if (!importStripPrefix) return fname;
        var normalized = fname.replace(/\\/g, '/');
        var prefix = importStripPrefix + '/';
    ...
    // Also strip the install prefix from manifest file lists so DB records
    // store paths relative to the install directory (not including it).
    filteredLibFiles = filteredLibFiles.map(function(f) { return stripImportPrefix(f); });
    helpFiles = helpFiles.map(function(f) { return stripImportPrefix(f); });
    demoFiles = demoFiles.map(function(f) { return stripImportPrefix(f); });
    ...
    zip.getEntries().forEach(function(entry) {
        ...
        if (entry.entryName.startsWith('library/')) {
            var fname = stripImportPrefix(entry.entryName.substring('library/'.length));
            if (fname) {
            ...
            var fname2 = stripImportPrefix(entry.entryName.substring('demo_methods/'.length));
            if (fname2) {
            ...
            var fname3 = stripImportPrefix(entry.entryName.substring('help_files/'.length));
            if (fname3) {
