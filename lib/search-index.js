...
/**
 * Compute Jaccard-like overlap of two n-gram sets (arrays).
 * Returns 0-1.
 */
function ngramOverlap(aGrams, bGrams) {
	if (!aGrams.length || !bGrams.length) return 0;
	var setB = {};
	bGrams.forEach(function(g) { setB[g] = true; });
	var intersection = 0;
	aGrams.forEach(function(g) { if (setB[g]) intersection++; });
	var union = Object.keys(setB).length;
	// Count unique aGrams not in B
	var setA = {};
	aGrams.forEach(function(g) { setA[g] = true; });
	Object.keys(setA).forEach(function(g) { if (!setB[g]) union++; });
...
SearchIndex.prototype.search = function(textQuery, options) {
	options = options || {};
	var tagFilters = (options.tagFilters || []).map(function(t) { return (t || '').toLowerCase().trim(); }).filter(Boolean);
